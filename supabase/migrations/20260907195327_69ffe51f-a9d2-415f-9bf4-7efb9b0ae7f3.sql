...
-- Used exclusively by tests/e2e/99-teardown.spec.ts. Double-guarded: caller must
-- be a training admin AND the course title must start with 'E2E '.
...
BEGIN
  ...
  DELETE FROM public.block_marks WHERE block_id = ANY(v_blocks);
  GET DIAGNOSTICS v_n = ROW_COUNT; v_counts := v_counts || jsonb_build_object('block_marks', v_n);

  DELETE FROM public.block_comments WHERE block_id = ANY(v_blocks);
  GET DIAGNOSTICS v_n = ROW_COUNT; v_counts := v_counts || jsonb_build_object('block_comments', v_n);

  DELETE FROM public.question_bank_usages WHERE lesson_id = ANY(v_lessons);
  ...
  DELETE FROM public.scorm_packages WHERE lesson_id = ANY(v_lessons);
  ...
  DELETE FROM public.standard_links WHERE course_id = _course_id;
  ...
  DELETE FROM public.course_resources WHERE course_id = _course_id;
